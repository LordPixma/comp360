...
authRoutes.get('/oidc/google/callback', async (c) => {
    ...
    if (!user) {
        ...
        await c.env.DB.prepare('INSERT INTO users (id, email, name, created_at) VALUES (?, ?, ?, ?)').bind(userId, payload.email, payload.name, new Date().toISOString()).run();
        user = { id: userId, email: payload.email, name: payload.name };
    }
    // Get user's tenant and role
    const membership = await c.env.DB.prepare('SELECT * FROM memberships WHERE user_id = ? LIMIT 1').bind(user.id).first();
    if (!membership) {
        // Create a new tenant for first-time users
        const tenantId = await generateId();
        await c.env.DB.prepare('INSERT INTO tenants (id, name, region, created_at) VALUES (?, ?, ?, ?)').bind(tenantId, `${payload.name}'s Workspace`, 'US', new Date().toISOString()).run();
        await c.env.DB.prepare('INSERT INTO memberships (tenant_id, user_id, role, added_at) VALUES (?, ?, ?, ?)').bind(tenantId, user.id, 'owner', new Date().toISOString()).run();
